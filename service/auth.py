...
if primary_site_admin_tenant_id:
    # dummy is required on initialization because Tenants must start first, before Tokens and SK are ready, tenants code attempts to grab token later
    t = auth.get_service_tapis_client(tenant_id=primary_site_admin_tenant_id, tenants=tenant_cache, jwt='dummy', generate_tokens=False)
else:
    t = None
